...
import { IDevfile, devfileToDevWorkspace } from '../src';
...
describe('DevWorkspace API integration testing against cluster', () => {
    ...
    describe('Test Node DevWorkspace Api against local cluster', () => {
        conditionalTest('Test run the creation, retrieval, update and deletion of a devworkspace', isIntegrationTestEnabled, async (done: any) => {
            ...
            const devfile = yaml.load(fs.readFileSync(__dirname + '/fixtures/sample-devfile.yaml', 'utf-8')) as IDevfile;
            ...
            var dwMeta = newDevWorkspace.metadata as any
            expect(dwMeta.name).toBe(name);
            expect(dwMeta.namespace).toBe(namespace);
            ...
            var dwMeta = firstDevWorkspace.metadata as any
            expect(dwMeta.name).toBe(name);
            expect(dwMeta.namespace).toBe(namespace);
        ...
        conditionalTest('Test run the creation, retrieval and deletion of a devworkspace template', isIntegrationTestEnabled, async (done: any) => {
            ...
            var dwtMeta = dwt.metadata as any
            const name = dwtMeta.name;
            const namespace = dwtMeta.namespace;
            ...
            var dwtMeta = newDWT.metadata as any
            expect(dwtMeta.name).toBe(name);
            expect(dwtMeta.namespace).toBe(namespace);
            ...
            var dwtMeta = firstTemplate.metadata as any
            expect(dwtMeta.name).toBe(name);
            expect(dwtMeta.namespace).toBe(namespace);

            const singleNamespace = await nodeApi.templateApi.getByName(namespace, name);
            var dwtMeta = singleNamespace.metadata as any
            expect(dwtMeta.name).toBe(name);
            expect(dwtMeta.namespace).toBe(namespace);
